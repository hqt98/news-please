    ...
    def process_item(self, item, spider):
        if spider.name in ["RssCrawler", "GdeltCrawler"]:
            ...
            if old_version is not None and (
                datetime.datetime.strptime(item["download_date"], "%y-%m-%d %H:%M:%S")
                - old_version[3]
...
class ExtractedInformationStorage(object):
    ...
    def extract_relevant_info(item):
        ...
        article = {
            ...
            "source_domain": ExtractedInformationStorage.ensure_str(
                item["source_domain"]
            ),
            "maintext": item["article_text"],
            "markdown": item["article_markdown"],
            "url": item["url"],
            "html": BeautifulSoup(item["spider_response"].body, "lxml").prettify(),
        }
    ...
    def convert_to_class(item):
        ...
        news_article.markdown = item["markdown"]
        news_article.html = item["html"]
        return news_article
...
class PostgresqlStorage(ExtractedInformationStorage):
    ...
    def __init__(self):
        ...
        options = (
            f"-c search_path={self.database.get('schema')}"
            if self.database.get("schema")
            else ""
        )
        # Establish DB connection
        # Closing of the connection is handled once the spider closes
    ...
    def process_item(self, item, spider):
        ...
        if old_version is not None:
            ...
            try:
                self.cursor.execute(
                    self.delete_from_current, (old_version_list["db_id"],)
                )
                self.conn.commit()
            except psycopg2.DatabaseError as error:
                self.log.error("Something went wrong in delete: %s", error)
